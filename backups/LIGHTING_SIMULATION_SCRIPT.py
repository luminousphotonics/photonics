...
# ------------------------------------
# Profile 0: Original Bare COB Data (~120 deg FWHM)
COB_ANGLE_DATA_BARE = np.array([
    [  0, 1.00], [ 10, 0.98], [ 20, 0.95], [ 30, 0.88], [ 40, 0.78],
    [ 50, 0.65], [ 60, 0.50], [ 70, 0.30], [ 80, 0.10], [ 90, 0.00],
], dtype=np.float64)
COB_angles_deg_bare = COB_ANGLE_DATA_BARE[:, 0]
COB_shape_bare = COB_ANGLE_DATA_BARE[:, 1]

# Profile 1: 90-degree Lens Data (Extracted from Plot)
COB_ANGLE_DATA_90 = np.array([
    [  0, 1.00], [ 10, 0.98], [ 20, 0.92], [ 30, 0.79], [ 40, 0.60],
    [ 45, 0.50], [ 50, 0.38], [ 60, 0.15], [ 70, 0.02], [ 75, 0.00],
    [ 80, 0.00], [ 90, 0.00],
], dtype=np.float64)
COB_angles_deg_90 = COB_ANGLE_DATA_90[:, 0]
COB_shape_90 = COB_ANGLE_DATA_90[:, 1]
...
print("\nCalculating COB Normalization Integrals...")
COB_NORMALIZATION_INTEGRAL_BARE = calculate_cob_normalization_integral(COB_angles_deg_bare, COB_shape_bare)
print(f"[INFO] Bare COB (~120deg) Normalization Integral: {COB_NORMALIZATION_INTEGRAL_BARE:.4f}")

COB_NORMALIZATION_INTEGRAL_90 = calculate_cob_normalization_integral(COB_angles_deg_90, COB_shape_90)
print(f"[INFO] 90deg Lens Normalization Integral: {COB_NORMALIZATION_INTEGRAL_90:.4f}")
print("-" * 20)

# Store profiles for Numba
# Profile 0 = Bare COB (~120 degree)
# Profile 1 = 90 degree Lens
ALL_LENS_PROFILES_DATA = [
    (COB_angles_deg_bare, COB_shape_bare, COB_NORMALIZATION_INTEGRAL_BARE),
    (COB_angles_deg_90, COB_shape_90, COB_NORMALIZATION_INTEGRAL_90),
]

# Create separate structures suitable for Numba parallel=True
# Use numba.typed.List for lists containing NumPy arrays
all_angles_list = List()
all_shapes_list = List()
_integrals_temp = [] # Temporary Python list

for angles, shape, integral in ALL_LENS_PROFILES_DATA:
    all_angles_list.append(np.array(angles, dtype=np.float64))
    all_shapes_list.append(np.array(shape, dtype=np.float64))
    _integrals_temp.append(integral)

# Convert integrals to a simple NumPy array
all_integrals_array = np.array(_integrals_temp, dtype=np.float64)

print(f"[INFO] Prepared separate lens profile data structures for Numba.")
...
def build_all_light_sources(W, L, H, cob_lumen_params, apply_transmittance_loss=True, transmittance_factor=0.94):
    """
    Builds COB light sources, assigns lens profile indices based on layer,
    and optionally adjusts lumens for lens transmittance.
    """
    ...
    if num_total_layers == 0 :
        ...
        return np.empty((0,3)), np.empty((0,)), np.empty((0,4)), np.empty((0,), dtype=np.int64) # Pos, Lumens, Pos+Layer, ProfileIdx
    ...
    light_profile_indices_list = [] # NEW: Stores index (0 or 1) of lens profile

    # Determine the index of the outermost layer that will actually have COBs placed
    active_layers = [i for i, lumens in enumerate(cob_lumen_params) if lumens > EPSILON]
    if not active_layers:
         outermost_layer_index = -1
         print("[Warning] All cob_lumen_params are zero. No lights will be generated.")
    else:
         max_active_layer_index = max(active_layers)
         max_abstract_layer = 0
         if abstract_cob_coords:
             max_abstract_layer = max(c['layer'] for c in abstract_cob_coords)
         outermost_layer_index = min(max_active_layer_index, max_abstract_layer)
         print(f"[Geometry] Outermost layer with active COBs identified as layer: {outermost_layer_index}")

    print("[Geometry] Placing COB emitters, assigning lens profiles, and adjusting lumens...")
    num_adjusted_lumens = 0
    for i, abs_cob in enumerate(abstract_cob_coords):
        layer = abs_cob['layer']
        if not (0 <= layer < num_total_layers):
            # print(f"[Debug] Skipping abstract COB with invalid layer {layer}") # Optional Debug
            continue

        lumens_nominal = cob_lumen_params[layer] # Get nominal lumens for this layer
        if lumens_nominal > EPSILON:
            ...
            # --- Assign Lens Profile Index ---
            if layer == outermost_layer_index and outermost_layer_index >= 0:
                profile_index = 1 # Use 90-degree lens (Profile 1)
                # --- Adjust Lumens for Transmittance ---
                if apply_transmittance_loss:
                    lumens_actual = lumens_nominal * transmittance_factor
                    num_adjusted_lumens += 1
                else:
                    lumens_actual = lumens_nominal
            else:
                profile_index = 0 # Use Bare COB profile (Profile 0)
                lumens_actual = lumens_nominal # No lens, no transmittance loss

            light_profile_indices_list.append(profile_index)
            all_lumens_list.append(lumens_actual) # Append the actual (potentially adjusted) lumens
            # --------------------------------------

    # Convert lists to numpy arrays
    try:
        if not all_positions_list:
            print("[Warning] No valid COB positions generated.")
            light_positions, light_lumens = np.empty((0, 3)), np.empty(0)
            cob_positions_only = np.empty((0, 4))
            light_profile_indices = np.empty((0,), dtype=np.int64)
        else:
            ...
            light_profile_indices = np.array(light_profile_indices_list, dtype=np.int64)

            num_profile_0 = np.count_nonzero(light_profile_indices == 0)
            num_profile_1 = np.count_nonzero(light_profile_indices == 1)
            print(f"[INFO] Assigned {num_profile_0} COBs with Profile 0 (Bare COB ~120deg).")
            print(f"[INFO] Assigned {num_profile_1} COBs with Profile 1 (90deg Lens - Outer Layer).")
            if apply_transmittance_loss and num_adjusted_lumens > 0:
                print(f"[INFO] Applied transmittance factor ({transmittance_factor:.2f}) to {num_adjusted_lumens} outer layer COBs.")
            elif apply_transmittance_loss:
                 print(f"[INFO] Transmittance adjustment enabled, but no outer layer COBs found or assigned.")
    ...
    print(f"[INFO] Generated {num_cobs_placed} COB emitters with potentially adjusted lumens.")

    # Return positions, ACTUAL lumens, detailed COB positions, AND profile indices
    return light_positions, light_lumens, cob_positions_only, light_profile_indices

def prepare_geometry(W, L, H, cob_lumen_params):
    """Prepares all geometry: COB lights (with profile indices & adjusted lumens), floor grid, patches."""
    print("[Geometry] Building COB light sources...")
    # Now returns 4 items from build_all_light_sources
    # Decide here if you want to apply transmittance loss (True/False)
    apply_loss = True
    light_positions, light_lumens, cob_positions_only, light_profile_indices = build_all_light_sources(
        W, L, H, cob_lumen_params, apply_transmittance_loss=apply_loss
    )
    ...
    # Return geometry including the profile indices and potentially adjusted lumens
    return light_positions, light_lumens, light_profile_indices, cob_positions_only, X, Y, patches_dict
...
def compute_direct_floor(light_positions, light_lumens, light_profile_indices, X, Y,
                         all_angles_list, all_shapes_list, all_integrals_array): # Pass separate data
    """ Computes direct floor illuminance using assigned COB lens profiles. """
    min_dist2 = (FLOOR_GRID_RES / 4.0) ** 2
    rows, cols = X.shape
    out = np.zeros_like(X)
    num_lights = light_positions.shape[0]

    # Numba implicitly handles access to these lists/arrays in parallel loops
    ...
    for r in prange(rows):
        for c in range(cols):
            ...
            for k in range(num_lights):
                lx, ly, lz = light_positions[k, 0:3]
                lumens_k = light_lumens[k]

                if lumens_k < EPSILON: continue

                # --- Select Lens Profile ---
                profile_idx = light_profile_indices[k]
                # Access data from the separate lists/array using the index
                cob_angles_deg_k = all_angles_list[profile_idx]
                cob_shape_k = all_shapes_list[profile_idx]
                cob_norm_integral_k = all_integrals_array[profile_idx]
                # ---------------------------

                dx, dy, dz = fx - lx, fy - ly, fz - lz
                d2 = max(dx*dx + dy*dy + dz*dz, min_dist2)
                dist = math.sqrt(d2)
...
@njit(parallel=True, cache=True)
def compute_patch_direct(light_positions, light_lumens, light_profile_indices,
                         patch_centers, patch_normals, patch_areas,
                         all_angles_list, all_shapes_list, all_integrals_array): # Pass separate data
    """ Computes direct patch illuminance using assigned COB lens profiles. """
    min_dist2 = (FLOOR_GRID_RES / 4.0) ** 2
    Np = patch_centers.shape[0]
    out = np.zeros(Np)
    num_lights = light_positions.shape[0]

    # Numba implicitly handles access
...
# ------------------------------------
def simulate_lighting(light_positions, light_lumens, light_profile_indices, X, Y, patches_dict):
    """ Runs the full lighting simulation pipeline (COBs with assigned lens profiles, MC Form Factors). """
    ...
    if num_patches == 0:
        print("[Error] Essential patch arrays are empty after fetching. Cannot simulate.")
        return np.zeros_like(X, dtype=np.float64) if X is not None else None
    ...
    if light_positions.shape[0] == 0:
        ...
        print("[Simulation] Calculating direct floor illuminance (Assigned COB Profiles)...")
        start_direct_floor_time = time.time()
        # Pass the separate lists/array defined globally
        floor_lux_direct = compute_direct_floor(
            light_positions, light_lumens, light_profile_indices,
            X, Y,
            all_angles_list, all_shapes_list, all_integrals_array # Pass new arguments
        )
        end_direct_floor_time = time.time()
        print(f"  > Direct floor finished in {end_direct_floor_time - start_direct_floor_time:.2f} seconds.")

        print("[Simulation] Calculating direct patch illuminance (Assigned COB Profiles)...")
    ...
    if form_factor_matrix is None or np.any(np.isnan(form_factor_matrix)) or form_factor_matrix.shape != (num_patches, num_patches):
        print("[ERROR] Invalid Form Factor matrix! Check MC sampling or geometry. Aborting indirect.")
        ...
        # Timing printed inside function
    ...
    if np.any(~np.isfinite(floor_ppfd)):
        print("[Warning] Non-finite values detected in final floor_ppfd. Replacing with 0.")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Lighting Simulation (COBs, Optional Lenses, MC Form Factors)")
    ...
    # Add argument to control transmittance loss application
    parser.add_argument('--no-lens-loss', action='store_true', help='Disable the ~6% lumen loss for outer 90deg lenses')
    args = parser.parse_args()

    N_FF_SAMPLES = args.ffsamples
    MC_SAMPLES = args.mcsamples
    apply_transmittance_loss_main = not args.no_lens_loss # Apply loss unless flag is set

    print(f"[Config] Using N_FF_SAMPLES = {N_FF_SAMPLES}, MC_SAMPLES = {MC_SAMPLES}")
    print(f"[Config] Applying 90deg lens profile (extracted) to outermost active COB layer.")
    print(f"[Config] Applying Bare COB profile (~120deg) to inner layers.")
    if apply_transmittance_loss_main:
         print(f"[Config] Applying estimated transmittance loss (x{0.94:.2f}) to 90deg lensed COBs.")
    else:
         print(f"[Config] Transmittance loss for 90deg lenses DISABLED via --no-lens-loss.")


    # --- Geometry ---
    print("\nPreparing geometry...")
    start_geom_time = time.time()
    # Pass the argument controlling loss application to prepare_geometry->build_all...
    # It now returns the potentially adjusted light_lumens and the indices
    light_positions, light_lumens, light_profile_indices, cob_positions_only, X, Y, patches_dict = prepare_geometry(
        W, L, H, cob_lumen_params
        # Note: Loss application is handled inside prepare_geometry/build_all_light_sources now
        # based on the apply_transmittance_loss_main flag logic derived from args
    )
    ...
    if num_patches == 0:
        ...
        return
    if total_lights == 0:
        print("[Warning] No light sources were generated. Simulation might be empty.")


    # --- Simulation ---
    print(f"\nStarting simulation: {total_lights} COB emitters (assigned profiles/lumens), {num_patches} patches...")
    start_sim_time = time.time()
    # Pass light_profile_indices to the simulation function
    floor_ppfd = simulate_lighting(light_positions, light_lumens, light_profile_indices, X, Y, patches_dict)
    ...
    print(f"Floor Grid: {X.shape[1]} x {X.shape[0]} points (~{FLOOR_GRID_RES:.3f} m)")
    print(f"Emitters: {total_lights} COBs")
    print(f"Lens Config: Outer layer used 90deg Lens Profile, Inner used Bare COB Profile.")
    ...
    # --- CSV ---
    csv_filename = "ppfd_layer_data_mixed_lenses.csv" # Changed filename
    ...
    if not args.no_plot:
        ...
        if np.all(np.isfinite(floor_ppfd)):
            ...
            print("[Plot Error] Cannot plot non-finite PPFD data.")
    else:
