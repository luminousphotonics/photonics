...
def run_simulation():
    # --------------------------------------------------
    # Read GUI Parameters with Error Handling
    # --------------------------------------------------

    def read_entries(entries, error_message):
        values = []
        try:
            for entry in entries:
                value_str = entry.get().strip()
                if not value_str:
                    raise ValueError("Input cannot be empty.")  # Check for empty strings
                values.append(float(value_str))
        except ValueError as e:
            messagebox.showerror("Input Error", f"{error_message}\n{e}") #More specific error message
            return None  # Return None on error
        return values

    layer_intensities = read_entries(layer_intensity_entries, "Error reading layer intensities:")
    if layer_intensities is None: return

    led_intensities = []
    for intensity, count in zip(layer_intensities, LAYER_COUNTS):
        led_intensities.extend([intensity] * count)

    corner_values = read_entries(corner_intensity_entries, "Error reading corner intensities:")
    if corner_values is None: return
    ...
    led_strip_fluxes = read_entries(led_strip_entries, "Error reading LED strip luminous flux values:")
    if led_strip_fluxes is None: return
    # --------------------------------------------------
    ...
    try:
        ...
        messagebox.showerror("File Error", f"Error loading SPD data:\n{e}") #More specific error message
        return
    ...
    if len(led_intensities) != N_cob_leds:
        messagebox.showerror("Input Error",f"Expected {N_cob_leds} intensities, got {len(led_intensities)}") #Error check
        return
    ...
    def reflectance_floor(wavelength):  #Added floor reflectance definition
        return 0.0
    ...
    A_total_refl = area_ceiling + area_walls

    R_eff = (rho_ceiling_eff * area_ceiling + rho_wall_eff * area_walls) / A_total_refl
    print(f"Effective wall reflectance = {rho_wall_eff:.3f}")
    print(f"Effective ceiling reflectance = {rho_ceiling_eff:.3f}")
    print(f"Effective room reflectivity (R_eff) = {R_eff:.3f}")

    irradiance_direct = direct_irradiance
    irradiance_reflected = direct_irradiance * R_eff / (1 - R_eff)
    total_irradiance = irradiance_direct + irradiance_reflected
    ...
    plt.title('Simulated PPFD on Floor\n(SPD Conversion, Radiosity-based Reflections)')
    
    # Optionally remove the stats_text box by omitting it.
    ...
    # Label LED strips with their IDs at the midpoint of each strip
    strip_midpoints = {}
    for strip_num, led_ids in led_strips.items():
    ...
    plt.show()
    #plt.close() #Close to prevent GUI issues
