...
# Define a dataclass for fixture definitions
@dataclass
class Fixture:
    file_path: str
    type: str
    ...
    index: int = -1  # Add an index attribute
...
def determine_fixture_placement(layer: int, fixture_index: int, center_x: float, center_y: float, base_layer_spacing: float) -> Fixture:
    ...
    if layer == 0:
        # Layer 1: Center Unit
        return Fixture(
            file_path="/Users/austinrouse/modeltest2.obj",
            type="X-Unit",
            layer=layer,
    ...
    # Layer 2: 2 Fixtures (L-Shaped)
        if fixture_index == 0:  # Bottom-left
            return Fixture(
                file_path="/Users/austinrouse/modeltest5.obj",
                type="L-Shaped",
                layer=layer,
        ...
        if fixture_index == 0:  # Top
            return Fixture(
                file_path="/Users/austinrouse/modeltest4.obj",
                type="Linear",
                layer=layer,
            ...
            return Fixture(
                file_path="/Users/austinrouse/modeltest14.obj",
                type="Linear",
                layer=layer,
        ...
        if fixture_index == 0:  # Bottom-right
            return Fixture(
                file_path="/Users/austinrouse/modeltest11.obj",
            ...
            return Fixture(
                file_path="/Users/austinrouse/modeltest12.obj",
                type="ReverseL-Shaped",
                layer=layer,
            ...
            return Fixture(
                file_path="/Users/austinrouse/modeltest13.obj",
                type="ReverseL-Shaped",
                layer=layer,
...
if __name__ == "__main__":
    ...
    center_y = floor_length / 2

    # Scaling factors
    global_scale = 0.07  # Adjust this to scale the heatmap and fixtures down
    plant_canopy_scale = 1.0  # Keep the plant canopy size unchanged

    # Spacing and height adjustments
    layer_spacing = 900.0 * global_scale  # Distance between fixtures
    lighting_height = 900.0 * global_scale  # Height of the lighting array above the heatmap
    canopy_height = 0  # Keep the canopy height consistent
    plant_spacing = 2.0  # Space between plant models
    plant_scale = 1.5  # Scale factor for plant models
    wall_height = 20.0 * global_scale  # Height of the enclosure walls

    # Path to realistic plant OBJ model
    plant_model_path = "/Users/austinrouse/Downloads/cannabis-sativa-plant/source/sativa_plant_fixed.obj"  # Replace with actual path

    # Generate fixtures and models
    fixture_definitions = generate_all_fixtures(floor_width, floor_length, center_x, center_y, layer_spacing)
    models = load_and_arrange_models(fixture_definitions, lighting_height)

    # Scale fixtures
    for model in models:
        model.scale(global_scale, inplace=True)

    # Generate heatmap and plane
    grid_size = (10, 10)  # Adjust for the density of plants
    _, _, heatmap = generate_heatmap(grid_size)
    heatmap_bounds = [0, floor_width * global_scale, 0, floor_length * global_scale, 0, 0]
    heatmap_plane = create_heatmap_plane(grid_size, heatmap, heatmap_bounds)

    # Populate canopy with realistic plant models, centered on the heatmap
    plant_models = populate_canopy_with_realistic_plants(
        center_x * global_scale,  # Scaled center coordinates
        center_y * global_scale,
        floor_width * global_scale,  # Scaled width and length of the heatmap
        floor_length * global_scale,
        canopy_height,
        grid_size,
        plant_model_path,
    ...
    # Visualization with walls
    plotter = pv.Plotter()

    # Add the heatmap
    plotter.add_mesh(heatmap_plane, scalars="heatmap", cmap="viridis", opacity=0.8, show_edges=False)

    # Add all lighting models
    for model in models:
        plotter.add_mesh(model, color="white", opacity=0.9, show_edges=True)
    
    # Add the plant canopy - now handled in the visualization function
    # for plant in plant_models:
    #     plotter.add_mesh(plant, color="green", opacity=1.0)
    ...
    # Use the visualization function that includes the plant canopy
    visualize_heatmap_with_realistic_plants(
        heatmap_plane,
        models,
        heatmap_bounds,
        center_x * global_scale,
        center_y * global_scale,
        canopy_height,
        plant_models,
        colormap="viridis"
