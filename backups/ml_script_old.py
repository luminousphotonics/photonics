...
def objective_function_BO(**kwargs):
    global geo_opt
    params = [kwargs[f'p{i}'] for i in range(6)]
...
def optimize_lighting(W, L, H, target_ppfd, progress_callback=None):
    ...
    pbounds = {f'p{i}': (1000, 24000) for i in range(6)}
    ...
    optimizer.probe(
        params={f'p{i}': val for i, val in enumerate([5000, 3000, 5016.69, 8000, 18000.06, 24000])},
        lazy=True,
    )
    optimizer.maximize(
        init_points=10,
    ...
    best_params = [optimizer.max['params'][f'p{i}'] for i in range(6)]
    geo_final = prepare_geometry(W, L, H, FLOOR_GRID_RES_FINAL)
    final_ppfd_unscaled = simulate_lighting(best_params, geo_final)
    mean_ppfd_unscaled = np.mean(final_ppfd_unscaled)
    scaling_factor = target_ppfd / mean_ppfd_unscaled if mean_ppfd_unscaled > 0 else 1.0
    best_params_scaled = [p * scaling_factor for p in best_params]
    ...
    print(f"Mean PPFD: {mean_ppfd:.2f}, MAD: {mad:.2f}, RMSE: {rmse:.2f}, DOU: {dou:.2f}%")
...
def run_simulation(floor_width_ft=12.0, floor_length_ft=12.0, target_ppfd=1250.0, floor_height=3.0):
