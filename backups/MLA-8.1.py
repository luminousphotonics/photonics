...
We optimize 7 total parameters:
- 6 layer intensities (in lumens) for the main COB layers
- 1 common intensity (in lumens) for all 4 corner COBs
...
def pack_luminous_flux(params):
    """
    params => [layer1, layer2, layer3, layer4, layer5, layer6, common_corner]
    Fills the 61-COB array accordingly.
    The first 6 parameters are used for the main layers.
    The 7th parameter is used for all corner COBs.
    """
    LAYER_COUNTS = [1, 4, 8, 12, 16, 20]
    layers_vals = params[0:6]     # 6 main layer intensities
    common_corner = params[6]     # common intensity for all corners
...
# ------------------------------
# Direct Irradiance on Patches (with Patch-Normal Cosine)
# ------------------------------
...
def optimize_lighting(W, L, H, target_ppfd):
    ...
    # Set bounds: first 6 use MAX_LUMENS_MAIN; 7th uses MAX_LUMENS_CORNER
...
def run_simulation(floor_width_ft=12.0, floor_length_ft=12.0, target_ppfd=1250.0):
    ...
    # Optimize and simulate lighting
    ...
    print("\n[RESULT] 6 layer intensities + 1 common corner intensity (lumens):")
    for i, val in enumerate(best_params):
        print(f"   Param {i+1}: {val:.1f}")
    print(f"[RESULT] Mean PPFD={mean_ppfd:.1f}, MAD={mad:.2f}")

def main():
    # Always runs with hard-coded defaults and verbose output
    run_simulation()
