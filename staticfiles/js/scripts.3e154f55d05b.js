...
(function() {
    ...
    function init() {
      ...
      // Find 5 closest and assign circle + GSAP shift
      points.forEach(p => {
        p.closest = points
          .filter(o => o !== p)
...
// Begin Photonics Background

document.addEventListener('DOMContentLoaded', () => {
    const section = document.getElementById('spark-scroll-section');
    const canvas  = document.getElementById('spark-canvas');
    if (!section || !canvas) {
      console.error('Missing spark-scroll-section or spark-canvas');
      return;
    }
    const ctx = canvas.getContext('2d');
    let width, height;
    ...
    const sparks = Array.from({length:200}, () => new Spark());
    ...
    function animate() {
      ...
      scrollV *= 0.95;  // decay scroll influence
      requestAnimationFrame(animate);
    }
    animate();
  });