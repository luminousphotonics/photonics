...
def write_ppfd_to_csv(filename, floor_ppfd, X, Y, cob_positions, W, L):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            #else: # Point might fall exactly on the last boundary or outside
            #    # Check if it belongs to the last layer
            #    if np.abs(dist_to_center - sampling_boundaries[FIXED_NUM_LAYERS]) < EPSILON:
...
def main():
    W = 13.4112  # 30 ft in meters
    L = W  # 30 ft in meters
    H = 0.9144  # 3 ft in meters # CORRECTED height
    
    params = np.array([
    4051.5297,     # Center COB (Layer 0)
    8571.1990,     # Layer 1
    10538.3646,     # Layer 2
    10686.0199,     # Layer 3
    9747.1581,     # Layer 4
