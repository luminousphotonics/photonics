...
(PPFD) distribution on the floor. It uses Root Mean Squared Error (RMSE) for optimization.
...
LUMENS_TO_PPFD_CONVERSION = 5  # Increased precision
MAX_LUMENS = 20000.0
MIN_LUMENS = 2000.0
...
def calculate_reflected_ppfd(measurement_points, light_sources, light_intensities_by_layer, 
    ...
    reflected_fraction = 0.1  # Fraction of light reflected
    edge_threshold = min(floor_width, floor_length) * 0.05  # Define the zone near the edges as 5% of the smaller dimension
    total_reflected_light = 0

    for idx, point in enumerate(measurement_points):
        # Determine if the measurement point is near the edge of the floor
        ...
        if is_near_wall:
            # Calculate direct PPFD from light sources
            direct_ppfd = calculate_ppfd_at_point(point, light_sources, light_intensities_by_layer, height_from_floor)
            # Estimate the amount of reflected light based on the direct PPFD and reflection fraction
            reflected_light = direct_ppfd * reflected_fraction
            total_reflected_light += reflected_light

    # Apply the perimeter reflectivity to the total reflected light
    total_reflected_light *= perimeter_reflectivity
    # Distribute the total reflected light evenly across all measurement points
...
def calculate_intensity_and_rmse(floor_width, floor_length, height_from_floor, light_intensities_by_layer, 
    ...
    rmse = np.sqrt(np.mean((total_ppfd - mean_intensity) ** 2))

    return total_ppfd, rmse, mean_intensity
...
def objective_function(light_intensities_by_layer, floor_width, floor_length, height_from_floor,
    ...
    objective_value = rmse_scaled + ppfd_penalty_scaled + intensity_dev_penalty_scaled
    print(f"Phase: {phase}, RMSE: {rmse_scaled:.2f}, PPFD Penalty: {ppfd_penalty_scaled:.2f}, Intensity Deviation Penalty: {intensity_dev_penalty_scaled:.2f}")
...
def optimize_lighting(floor_width, floor_length, height_from_floor,
    ...
    _, rmse, optimized_ppfd = calculate_intensity_and_rmse(
        floor_width, floor_length, height_from_floor, optimized_intensities, 
        perimeter_reflectivity, measurement_points
    )

    if verbose:
        print(f"Optimization completed with RMSE: {rmse:.2f} and Optimized PPFD: {optimized_ppfd:.2f}")

    return optimized_intensities, rmse, optimized_ppfd
...
def run_simulation(floor_width, floor_length, target_ppfd, perimeter_reflectivity, verbose=False):
    ...
    optimized_intensities, rmse, optimized_ppfd = optimize_lighting(
    ...
    print(f"Minimized RMSE: {rmse:.2f} µmol/m²/s")
