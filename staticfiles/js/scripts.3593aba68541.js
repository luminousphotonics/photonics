...
(function() {
  ...
  function init() {
    ...
    // find closest & add circle
    points.forEach(p1 => {
      p1.closest = points
        .filter(p2 => p2!==p1)
        .sort((a,b)=>dist(p1,a)-dist(p1,b))
        .slice(0,5);
      p1.circle = { pos:p1, radius:2+Math.random()*2, active:0 };
      gsap.to(p1, {
        duration: 1 + Math.random(),
        x: p1.originX-50+Math.random()*100,
        y: p1.originY-50+Math.random()*100,
        ease: "circ.inOut",
        repeat: -1,
        yoyo: true
      });
  ...
  function drawCircle(c) {
    ...
    ctx.fillStyle = `rgba(156,217,249,${c.active})`;
    ctx.fill();
  }
  ...
  function onScroll() {
    animateHeader.val = (window.scrollY < height);
  }
})();
// End Intro BG Animation
