...
def calculate_ppfd_from_lumens(lumens, effective_area):
    ...
    ppfd = lumens / effective_area * 15 # Using 15 as the conversion factor: 1 μmol/s ≈ 15 lumens
    return ppfd
...
def prepare_heatmap_data():
    ...
    for layer_index, layer in enumerate(light_sources):  # Use enumerate to get the index
        for emitter in layer:
            # Check if the emitter is a tuple (for Diamond patterns) or a float (for Grid patterns)
            if isinstance(emitter, tuple):
                emitter_x, emitter_y = emitter
            else:
                emitter_x, emitter_y = emitter, 0.0  # Default y-value for grid patterns
...
def generate_2d_intensity_profile(center_x, center_y, measurement_points, intensity):
    """
    Generates a 2D intensity distribution profile (radial).

    Args:
        center_x: The x-coordinate of the center point.
        center_y: The y-coordinate of the center point.
        measurement_points: A list of (x, y, z) tuples representing measurement points.
        intensity: A list of intensity values corresponding to the measurement points.
    """
    radial_distances = []
    intensities = []
    ...
    for i, point in enumerate(measurement_points):
        x, y, _ = point
        radial_distance = math.sqrt((x - center_x)**2 + (y - center_y)**2)
    ...
    binned_intensities = [[] for _ in range(num_bins)]

    # Assign intensity values to bins
    for dist, inten in zip(radial_distances, intensities):
        for i in range(num_bins):
            if bin_edges[i] <= dist < bin_edges[i+1]:
                binned_intensities[i].append(inten)
                break
    ...
    bin_centers = []
    for i in range(num_bins):
        if binned_intensities[i]:
            average_intensities.append(np.mean(binned_intensities[i]))
        else:
            average_intensities.append(0)  # Handle empty bins
        bin_centers.append((bin_edges[i] + bin_edges[i+1]) / 2)

    # Plot the 2D profile
    plt.figure(figsize=(8, 6))
    plt.plot(bin_centers, average_intensities)
    plt.xlabel("Radial Distance (feet)")
    plt.ylabel("Average PPFD")
    plt.title("2D Intensity Distribution Profile")
    plt.grid(True)
    plt.show()
